...
const HITL_NEGATIVE_RE =
  /^(none|n\/?a|no|not required|not needed|nothing|no input needed|no additional information)\s*\.?$/i;
...
function parseHitlQuestion(text: string): string | null {
  ...
  if (!question || HITL_NEGATIVE_RE.test(question)) { return null; }
  return question;
}
