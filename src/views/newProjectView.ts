...
export class NewProjectViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "pentag.newProjectView";
  private _view?: vscode.WebviewView;

  constructor(private readonly _context: vscode.ExtensionContext) {}

  /** Called by extension.ts when the workspace path changes via Settings */
  public notifyWorkspaceChanged(newPath: string | undefined): void {
    ...
    webviewView.webview.onDidReceiveMessage(
      async (message: { command: string; payload?: unknown }) => {
        switch (message.command) {
          case "createProject":
            await this._handleCreateProject(message.payload as ProjectConfig);
            break;
  ...
  private async _browseCollection(): Promise<void> {
  ...
  private async _handleCreateProject(config: ProjectConfig): Promise<void> {
    ...
    if (!workspacePath) {
      vscode.window.showErrorMessage(
        "AXIS Bot: No workspace folder configured. Please set it in the Settings panel.",
      );
      return;
    }
    ...
    try {
      if (!fs.existsSync(workspacePath)) {
        vscode.window.showErrorMessage(
          `AXIS Bot: Workspace folder does not exist: ${workspacePath}`,
        );
        return;
      }
      ...
      if (fs.existsSync(projectPath)) {
        const overwrite = await vscode.window.showWarningMessage(
          `Project folder "${folderName}" already exists. Overwrite config?`,
          "Overwrite",
          "Cancel",
        );
      ...
      fs.writeFileSync(
        path.join(projectPath, "proj_config.json"),
      ...
      vscode.window
        .showInformationMessage(
          `Project "${folderName}" created at ${projectPath}`,
          "Open Folder",
        )
        .then((choice) => {
      ...
      vscode.window.showErrorMessage(
        `AXIS Bot: Failed to create project -  ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
