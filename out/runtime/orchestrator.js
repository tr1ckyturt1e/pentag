 ...
 */
function emitVulns(text, context) {
 ...
 */
function buildFindingsSummary(results) {
    if (results.length === 0) {
...
function buildProjectContext(config) {
        ...
        config.apps.forEach((app, i) => {
            ...
            if (app.credentials.length > 0) {
                ...
                app.credentials.forEach((c) => {
                    lines.push(`    - Role: ${c.role}  |  Username: ${c.username}  |  Password: ${c.password}`);
                });
            }
            else {
    ...
    lines.push("");
    lines.push("> All information above is already known. Only use HUMAN_INPUT_REQUIRED: if you need" +
...
class Orchestrator {
    ...
    _modelService;
    _memoryStore;
    constructor(_agentLoader, _modelService, _memoryStore) {
        this._agentLoader = _agentLoader;
        this._modelService = _modelService;
        this._memoryStore = _memoryStore;
    }
    /**
     ...
     */
    async run(intent, context, sessionKey, onChunk) {
        ...
        for (let step = 0; step < MAX_DELEGATE_STEPS; step++) {
            ...
            let orchText;
            try {
                orchText = await this._singleTurn(model, orchAgent.systemPrompt, orchestratorInput, orchKey, "orchestrator", context, onChunk);
            }
            catch (err) {
                const status = err instanceof vscode.CancellationError ? "cancelled" : "failed";
            ...
            if (/^COMPLETE/m.test(orchText)) {
                ...
                if (!reportingAgentRan) {
                    ...
                    let reportResult;
                    try {
                        reportResult = await this._runReAct(model, REPORTING_AGENT_ID, forcedTask, sessionKey, context, onChunk);
                    }
                    catch (err) {
            ...
            try {
                specialistResult = await this._runReAct(model, delegateId, enrichedTask, sessionKey, context, onChunk);
            }
            catch (err) {
    ...
    // -- Inner ReAct loop -------------------------------------------------------
    async _runReAct(model, agentId, task, sessionKey, context, onChunk) {
        ...
        for (let turn = 0; turn < MAX_AGENT_TURNS; turn++) {
            ...
            let turnText;
            try {
                turnText = await this._singleTurn(model, agent.systemPrompt, currentInput, agentKey, agentId, context, onChunk);
            }
            catch (err) {
                ...
                if (!recoveredTurns.has(turn) && context.onHitlQuestion) {
                    ...
                    if (recovery === "skip") {
                        ...
                        return (lastText || `Agent ${agentId} was skipped at operator request.`);
                    }
                    if (recovery !== null) {
     ...
     */
    async _recoverFromError(agentId, err, context) {
        ...
        let reply;
        try {
            reply = await context.onHitlQuestion(agentId, `Agent encountered an error: "${errMsg}"\n\n` +
    ...
    // -- Single LLM turn --------------------------------------------------------
    async _singleTurn(model, systemPrompt, userInput, memoryKey, agentId, context, onChunk) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                let fullText = "";
                for await (const chunk of this._modelService.stream(model, messages, context.cancellationToken, tools, context.toolInvocationToken)) {
                    fullText += chunk;
                ...
                this._memoryStore.append(memoryKey, vscode.LanguageModelChatMessage.User(userInput));
                this._memoryStore.append(memoryKey, vscode.LanguageModelChatMessage.Assistant(fullText));
                return fullText;
            }
            catch (err) {
                if (err instanceof vscode.CancellationError) {
